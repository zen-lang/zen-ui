    ...
    (binding [*handling* true]
      (loop []
        (when-let [queue-head (dequeue! queue)]
          (do
            (klog.core/log :rf/ev {:rf queue-head})
            (when-not (re-frame.registrar/get-handler :event (first queue-head))
              (throw (Exception. (str "No handler for " queue-head))))
...
(alter-var-root (var re-frame.subs/cache-lookup)
                (fn [_] (fn [& args] #_(println ">>> Cache lookup" args))))
